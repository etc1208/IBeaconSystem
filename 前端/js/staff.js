    ...
    $.getJSON(
        ...
        function( result ) {
            ...
            if($.isArray(arr)){
                ...
                //添加代理的Beacon数量
                var sessions = result.staff[0].session;
                var device = result.staff[0].device;
    			var $tr = null;
    			var devcount = 0;
    			$('#collapse2_table tbody').empty();
    			for(var i=0;i<sessions.length;i++){
    				var uuid = sessions[i].value;
    				var majors = sessions[i].majors;
    				for(var j=0;j<majors.length;j++){
    					var major = majors[j].value;
    					var minors = majors[j].sections;
    					for(var k=0;k<minors.length;k++){
    						var minor = minors[k].value0 + '-' + minors[k].value1;
    						//alert(minor);
    						var bushu = device[devcount].bushu;
    						var waitbushu = device[devcount].waitbushu;
    						var peizhi = device[devcount].peizhi;
    						var huishou = device[devcount].huishou;
    						var all = device[devcount].all;
    						var url = device[devcount].url;
    						$tr = $('<tr><td>'+uuid+'</td><td>'+major+'</td><td>'+minor+'</td><td>'+all+'</td><td>'+bushu+'</td><td>'+waitbushu+'</td><td>'+peizhi+'</td><td>'+huishou+'</td><td>'+url+'</td></tr>');
    						$('#collapse2_table tbody').append($tr);	
    						devcount++;
    					}
    				}
    			}
...
function showSession(){
	var V = new validateFormInput($('#showSession'));
    V.destroy();
    $("#showSession").modal("show");
    //$("#Show").html("");
    $("#uuids").val("");
    $("#majors").val("");
    $("#value0s").val("");
...
function showS(){
	...
	if(V.validate()){
		...
		$('#showSession').block({ message: '<img src="./images/busy.gif" />' });
		$('#staff2table').DataTable().destroy();
		$('#staff2table tbody').empty();
		var url = "http://"+publicElement.ip+":"+publicElement.port+"/beacon/staff!findonebysession?jsonstr="+jsonstr+"&jsoncallback=?";
		$.getJSON(
                url,
                function( result ) {
                    if (!result.success) {
                    ...
                    for ( var i = 0; i < result.size; i++ ) {
                        ...
                        $('#staff2table tbody').append($tr);
                        
                    }
                    
                    $.unblockUI();
                    $('#staff2table').dataTable(
                        {
                            "order": [ 3, 'desc' ],
                            "columns": [
           				             { "width": "15%" },
