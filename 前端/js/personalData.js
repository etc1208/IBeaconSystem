...
$(function(){
	...
	$('#personalData').on('click',function(){
		...
		V.destroy();
		var  url = "http://"+publicElement.ip+":"+publicElement.port+"/beacon/staff!findone?user_id="+uid+"&jsoncallback=?";
	    ...
	    $.getJSON(
	        ...
	        function( result ) {
	            ...
	            publicElement.parent_id = result.staff[0].parent_id;
	            ...
	            publicElement.session = result.staff[0].session;
	            if($.isArray(session)){
...
//console.log(publicElement.session);
    ...
    if(V.validate()){
    	jsonstr.push({
    		...
    		parent_id:publicElement.parent_id,
            session:publicElement.session
        });
