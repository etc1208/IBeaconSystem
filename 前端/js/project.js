	...
	$.getJSON(
		...
		function(result){
			...
			for(var i=0;i<result.total;i++){
				...
				$tr = $("<tr><td>"+title+"</td><td>"+address+"</td><td>"+time+"</td><td>"+begin+"</td><td>"+end+"</td><td>"+description+"</td><td><button type='button' class='btn btn-sm btn-primary' projectId='"+project_id+"'>详情</button> <button type='button' class='btn btn-sm btn-warning' projectId='"+project_id+"'>修改</button></td></tr>");
				$('#projectTable tbody').append($tr);
				
				$tr.on('click',function(e){
					$target = $(e.target);
					if($target.hasClass('btn-warning') || $target.parent().hasClass('btn-warning')){
...
//======================================================
//功能： 显示指定项目的相关详情，包括该项目下使用的设备数，url数等
//======================================================
function showProjectDetails(projectId){
	var url = "http://"+publicElement.ip+":"+publicElement.port+"/beacon/project!showDetails?project_id="+projectId+"&jsoncallback=?";
	$.getJSON(
		url,
		function(result){
			if(!result.success){
			...
			$('#projectDetails_beacon').val(result.details[0].Beacon);
			$('#projectDetails_url').val(result.details[0].url);
			$('#projectDetails_ID').val(result.details[0].ID);
